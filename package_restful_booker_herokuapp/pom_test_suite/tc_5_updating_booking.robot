...
#    Fail                Failing the text case   Deprecated  Smoke Test
    ...
    When KW: Update Booking PUT Session.
    
    Given KW: Interacting with the Excel sheet2.
    Then KW: Request PUT Header.

    And KW: PUT Body with JSON Keys & Value.

    Then KW: Interacting with the Excel sheet1.
    Then KW: Fetching the last value of the saved ids from the sheet1.

    run keyword and continue on failure        KW: Response of the PUT Request.
    ...
    ${total price value}=     Get Value From Json       ${json}       $.totalprice
    # ${total price value get from list}=    get from list   ${total price value}   0
    ${total_price_to_String}=     convert to string       ${total price value}
    Write To Cell       D${Next_Row_Evalute}        ${total_price_to_String}

    ${depositpaid_true_or_false}=     Get Value From Json       ${json}       $.depositpaid
    # ${depositpaid_true_or_false get from list}=    get from list   ${depositpaid_true_or_false}   0
    ${true_or_false_to_String}=     convert to string       ${depositpaid_true_or_false}
    Write To Cell       E${Next_Row_Evalute}        ${true_or_false_to_String}

    ${checkin value}=     Get Value From Json       ${json}       $.bookingdates.checkin
    # ${checkin value get from list}=    get from list   ${checkin value}   0
    ${checkin_value_to_String}=     convert to string       ${checkin value}
    Write To Cell       F${Next_Row_Evalute}        ${checkin_value_to_String}

    ${checkout value}=     Get Value From Json       ${json}       $.bookingdates.checkout
    # ${checkout value get from list}=    get from list   ${checkout value}   0
    ${checkout_value_to_String}=     convert to string       ${checkout value}
    Write To Cell       G${Next_Row_Evalute}        ${checkout_value_to_String}

    ${additionalneeds_value}=     Get Value From Json       ${json}       $.additionalneeds
    # ${additionalneeds_value get from list}=    get from list   ${additionalneeds_value}   0
    ${additionalneeds_value_to_String}=     convert to string       ${additionalneeds_value}
    Write To Cell       H${Next_Row_Evalute}        ${additionalneeds_value_to_String}
