...
***Variables***
${top multi grid item place element xpath}                                        xpath\=(//*[name()\='rect'][@fill\='white'])[1]          #//span[normalize-space()\='place']
...
@{internal place type}=                Corporate All-Hands     Employee Onboarding     Employee Training       Hackathon       Leadership Meeting      Office Party
@{external place type}=                Conferences     Customer Advisory Board     Exhibitions     External Job Fair       Lead Generation Activities      Marketing Event
@{event type random list}=              Internal        External
${create new event text}                CREATE NEW EVENT
${new event text}                       New Event
${texts below no upcoming place}       Host a successful virtual event, and connect with your attendees globally in just a few steps, All you need to get started is here, Learn More
...
***Keywords***
KW: Expect texts on the place page.
    Run Keyword If      '${is text visible}'=='True'      Run Keyword     KW: Verify All    ${texts below no upcoming place}   ${waiting duration}
...
KW: Random internal radio value.
    KW: Get length      ${internal place type}
    ...
    KW: Get From List       ${internal place type}       ${eventtype_internal_random_NUMBER}
...
KW: Random external radio value.
    KW: Get length      ${external place type}
    ...
    KW: Get From List       ${external place type}       ${eventtype_external_random_NUMBER}
