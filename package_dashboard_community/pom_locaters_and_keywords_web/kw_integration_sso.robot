...
${palipoint icon plus sso icon}                                    ${sso_image}palipoint_plus_sso.png
...
KW: Expect the sso section element text is displayed.
    Is Text      Allows team members to sign in to your site using their Single Sign-On credentials. palipoint allows for Single Sign-On authentication via Google or SAML.
...
KW: Expect the sso side panel has text displayed.
    Is Text     palipoint allows for Single Sign-on authentication using Google, SAML and many more platforms.
...
KW: Verifying the palipoint plus the sso icon visibility.
    Verify Icon         ${palipoint icon plus sso icon}
...
KW: Verify the element has support text and email ids.
    Verify All        If you want to Create a New Authentication(with a new Organisation and Domain Name), reach out to, 'support@palipoint.com'.
